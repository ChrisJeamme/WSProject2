	...
	xhr.onload = function() {
		...
		if (xhr.status == 200) {
			...
			uploadSuccess.innerHTML = "<p>Photo Uploaded Successfully.</p>"
					+ "<img class=\"img-fluid\" src= \"" + response.downloadUri
					+ "\" alt=\"Uploaded photo\"/>"

					+ "<p>DownloadUrl : <a href='" + response.downloadUri
					+ "' target='_blank'>" + response.name + "</a></p>";
			uploadSuccess.style.display = "block";
		} else {
			uploadSuccess.style.display = "none";
			uploadError.innerHTML = (response && response.message)
					|| "Some Error Occurred";
		}
	}
	xhr.send(formData);
